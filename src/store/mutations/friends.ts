...
export default {
  ...
  async fetchFriends(state: any) {
    // @ts-ignore
    const ethereum = window.vault74provider;
    ...
    friendAddresses.forEach(async (f, i: number) => {
      ...
      if (!updateNeeded) { // If we already need to update, don't bother checking again
        const storedFriend = state.friends.filter((fr: IFriend) => fr.address === friend.address);
