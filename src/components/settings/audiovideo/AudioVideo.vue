      ...
      <div class="message-body">
        ...
        <div class="select">
          <select v-model="$store.state.audioDevice">
            <option v-for="device in audioDevices" :key="device.deviceId">{{
              device.label || $t('settings.audio_video.input_default_text')
            }}</option>
          </select>
        </div>
        <hr class="spacer" />
        ...
        <div class="select">
          <select>
            <option>{{
              $t('settings.audio_video.audio_output_default')
            }}</option>
          </select>
        </div>
        <hr class="spacer" />
        ...
        <div class="bordered margin-2">
          <p v-if="love >= 20">
            <i class="fa fa-heart red" /> Lauren my darling, I love you from now
            until forever.
          </p>
          <div class="columns no-mobile">
            <div class="column image-ipfs" style="max-width: 100px;">
              <img
                v-on:click="love += 1"
                src="static/img/icons/opus-logo.png"
                class="margin"
                alt=""
              />
            </div>
      ...
      <div class="message-body">
        ...
        <div class="select">
          <select v-model="$store.state.videoDevice">
            <option v-for="device in videoDevices" :key="device.deviceId">{{
              device.label || $t('settings.audio_video.video_default_text')
            }}</option>
          </select>
        </div>
      </div>
      ...
      <div class="message-body">
        ...
        <div class="select">
          <select v-model="$store.state.captureMouse">
            <option value="always">{{
              $t('settings.audio_video.capture-mouse-always')
            }}</option>
            <option value="motion">{{
              $t('settings.audio_video.capture-mouse-movement')
            }}</option>
            <option value="never">{{
              $t('settings.audio_video.capture-mouse-never')
            }}</option>
          </select>
        </div>
      </div>
