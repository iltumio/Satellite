<template>
  <div id="context" ref="fileMenu" v-click-outside="close">
    ...
    <hr class="divider" />
    <ul>
      <li v-on:click="openFile">{{ $t('file_context.open') }}</li>
      <li v-clipboard:copy="file.remote" v-on:click="closeSoon">
        {{ $t('file_context.copy_link') }}
      </li>
    </ul>
    <hr class="divider" />
    <ul>
      <li v-on:click="closeSoon" v-clipboard:copy="file.path">
        {{ $t('file_context.copy_id') }}
      </li>
    </ul>
  </div>
</template>
